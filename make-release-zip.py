...
def validate_package(root: Path) -> list[str]:
    validator = root / "validate-agent-harness-maintenance.py"
...
def build(root: Path, version: str) -> tuple[Path, Path, list[str]]:
    ...
    output = output_dir / f"beginners-guide-to-agent-harness-maintenance-{version}.zip"
...
def main() -> int:
    ...
    parser.add_argument("--version", required=True, help="Release version, such as v0.1.0.")
